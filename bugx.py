...
class Bug2(Node):
    ...
    def distance_to_line(self):
        ...
        p2x = self.goal_x
        p2y = self.goal_y
        br = abs(p2y - p1y) * p0x - (p2x - p1x) * p0y * (p2x * p1y)
        naz = math.sqrt((p2y - p1y) + (p2y -p1x))
        distance = br / naz
        return distance 
    ...
    def bug_algorithm_callback(self):
        ...
        elif self.state == 'the_goal':
            if abs(self.goal_x-self.current_x )and abs(self.goal_y-self.current_y) <0.1 :
                self.cmd_vel_msg.angular.z = 0.0
                self.cmd_vel_msg.linear.x = 0.0
